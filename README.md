...
管理员登录后仍使用与访客一致的网站界面，可在首页、地图和项目详情中直接编辑文字；每个非遗项目另有工序管理页，用于增删工序、材料、工具和操作。在线内容保存在服务器独立 JSON 文件中，原始证据、知识库与 3D 模型仍由 Git 管理。登录、协作、部署和备份说明见 [`docs/内容后台使用与部署.md`](docs/内容后台使用与部署.md)。
