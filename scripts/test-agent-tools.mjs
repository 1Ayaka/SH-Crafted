...
// The adapter no longer injects Git-bundled graph seeds. In this browserless
// unit test data.js has not loaded /api/v1/content, so the graph must stay
// empty instead of silently diverging from the server-authoritative graph.
assert.deepEqual(searchGraph('竹子', { limit: 8 }), []);
...
assert.equal(heritageDetailTarget('heritage:SHIH_0002'), null, 'API 尚未加载时不能伪造项目详情映射');
...
assert.deepEqual(heritageForGraphTarget('material:bamboo').nodes, []);
assert.equal(resolveIntent('竹子', { visible_nodes: [] }), null);
assert.equal(resolveIntent('打开竹材', { visible_nodes: [] }), null);
