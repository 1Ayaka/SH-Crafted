...
版本：v0.3 · 2026-08-12
范围：混合 RAG、ReAct 工具闭环、情境化探索建议、图谱兼容层、点击说话/唤醒适配层
...
- 根据用户话题生成可点击的关系星图与非遗项目入口，并在每次回答后给出两个情境化延伸问题。
- 模型可读取前端实时维护的工具清单，通过 Function Calling 提议动作；Tool Registry 校验后执行，并将结果回传模型组织最终回答。
- 原问题、去口语化关键词、图谱实体三路召回，经 RRF 融合、来源等级与审核状态重排，构成混合 RAG。
- ReAct 最多三轮、每轮最多三个工具；不向用户暴露隐藏推理，只展示正在查找或执行的动作。

当前图谱增强层只把代码和内容中已有的地区归属作为 `LOCATED_IN`。没有正式来源的传统/材料关系返回空状态；不能把 `category`、摘要文字或模型推断当作正式关系。

### 1.1 情境化探索建议

智能体采用“解释与行动分离”的混合规划方式：

1. 模型根据当前页面、知识库命中和候选节点解释用户的问题；
2. 前端通过 `graph-adapter` 检索真实公开节点，并把材料、传统或地区扩展为关联非遗；
3. Tool Registry 校验节点 ID 后，才允许按钮跳转到关系星图或非遗详情；
4. 无模型、超时或离线降级时，图谱入口和两个延伸问题仍然可用。

例如输入“竹子”，系统优先匹配 `material:bamboo`，显示“竹材关系星图”，并列出图谱中确实通过 `USES_MATERIAL` 关联的非遗项目。这只是回归测试用例；相同规划会扫描全部公开非遗、地区、传统和材料节点。查询具体非遗时，还会自动读取该项目已有的地区、材料和传统关系，不需要逐个项目编写推荐规则。

回答采用“项目资料优先、AI 通识补充”的策略。纪录片、知识库或图谱没有直接支持的段落允许生成更完整的背景解释，但必须以“AI生成”标注；不得借此虚构具体传承人、精确年份、名录等级、机构观点或来源链接。对地域成因可以从气候、原料、交通、市场和生活方式提出可能性，同时明确它是资料结论还是 AI 推测。

延伸问题不是预先写死的冷知识答案，而是根据当前问题类型生成的下一步检索入口：材料问题引向地域原因与同材料项目，地域问题引向环境影响与邻近项目，工艺问题引向相似材料和工序。具体回答仍由统一知识库和审核状态约束。
...
模型不能访问 DOM、构造 URL 或直接改变状态。它可以依据动态 Tool Manifest 生成结构化 Function Call；实际导航必须经过 Registry 的 JSON Schema、风险等级与节点存在性校验，再由页面宿主执行。
...
用户问题 + 当前页面状态
  → 混合 RAG 召回证据
  → 模型观察上下文并选择工具
  → Tool Registry 校验名称、参数、节点和风险
  → 页面宿主执行跳转/搜索/展开/朗读
  → 结构化结果回传模型
  → 模型根据执行结果回答或继续调用下一工具
```

这里的 ReAct 指 Reasoning + Acting。模型负责“选择下一步”，程序负责“能不能做、怎样安全地做”。当前上限为三轮，未知工具、额外参数、非法节点和任意 URL 均会被拒绝；模型服务不可用时仍保留确定性意图路由和本地检索回答。

### 2.2 混合 RAG 与证据回传

检索以带来源的知识片段为基本单位，不直接把整段长视频或整篇网页交给模型：

1. Query 通道保留用户完整问题；Keyword 通道去除口语成分；Graph Entity 通道识别项目或地区名称。
2. 各通道独立召回后使用 Reciprocal Rank Fusion 合并，避免单个关键词主导结果。
3. 当前项目命中、A/B/C 来源等级、`verified_external` 审核状态参与重排。
4. 模型只接收少量高相关片段；页面同时保留 `chunk_id`、来源、时间码和证据入口。
5. 模型不可用时，检索结果仍可独立形成回答，证据链不依赖生成模型。
...
当前默认：唤醒关闭；用户主动开启后，服务器本地 FunASR 以短片段识别“小蕉小蕉”，一次唤醒只处理一个问题，然后回到等待状态。当前唤醒前音频会发送到项目自己的服务器，因此属于服务器短语门控，不是浏览器离线 KWS；不保存原始音频，页面进入后台即暂停。浏览器 Speech Recognition 只作兼容降级，浏览器 TTS 用于朗读。

降级顺序：主动开启的 FunASR 唤醒 → FunASR 点击说话 → 浏览器 STT → 文字输入 → 静态帮助。未来可把第一层替换为本地 WASM KWS，不需改 Tool Registry。任何语音失败都不会阻塞地图、详情和知识图谱。

## 7. 参数与隐私

管理员/运维可调整：`VOICE_WAKE_WORDS`（逗号分隔，默认“小蕉小蕉”）、`deploy/funasr/hotwords.txt` 权重、FunASR 模型名、TTS 语速 0.6—1.4、提示音开关和搜索 limit 1—12。浏览器本地设置保存在 `sh-crafted.voice-preferences`，唤醒不默认开启；当前产品策略固定为一次唤醒一个问题，`continuousSeconds` 不用于免唤醒追问。

正式唤醒词现按项目要求设为“小蕉小蕉”，上线前仍须在安静、音乐、多人交谈、普通讲解环境测试误唤醒/漏唤醒。当前没有引入第三方唤醒 SDK；FunASR 镜像与模型许可证、服务器算力成本、转写保留政策仍需项目方正式确认。

## 8. 测试与评测

已有命令：`npm run check`、`npm run ui:test`、`npm run kb:test-api`、`npm run admin:test-ui`。智能体专项包括 `npm run agent:test-tools`、`npm run agent:test-react`、`npm run agent:test-recommendation` 和 `npm run agent:test-local-api`，覆盖未注册工具、非法 JSON、额外参数、非法关系、工具结果回传、推荐意图与无模型降级。
